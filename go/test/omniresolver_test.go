...
import (
	...
	omni "github.com/voxgig-sdk/ziptastic-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ZiptasticError); is {
				out := map[string]any{
					"name":    "ZiptasticError",
...
func omniCtx(arg any, client *sdk.ZiptasticSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ZiptasticSDK); is {
			client = live
