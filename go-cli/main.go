// Command Ziptastic-cli is a CLI + REPL for the Ziptastic SDK,
...
const prompt = "ziptastic"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_location_by_zipcode"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      ZIPTASTIC_APIKEY (api key), ZIPTASTIC_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: ZIPTASTIC_APIKEY carries the API key and
	// ZIPTASTIC_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ZIPTASTIC_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ZIPTASTIC_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewZiptasticSDK(opts)
