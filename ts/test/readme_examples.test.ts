...
//     (\`new ZiptasticSDK(...)\` / \`ZiptasticSDK.test(...)\` become
//     \`ZiptasticSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"get_location_by_zipcode":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ZiptasticSDK(...)\` and any
// \`ZiptasticSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ZiptasticSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ZiptasticSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
