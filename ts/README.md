...
The TypeScript SDK for the Ziptastic API. Provides a type-safe, entity-oriented interface with full async/await support.
...
```ts
const result = await client.GetLocationByZipcode().load({ id: 'example_id' })
...
```ts
const client = new ZiptasticSDK({ apikey: '...' })
