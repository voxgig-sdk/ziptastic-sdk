...
[MCP](https://modelcontextprotocol.io) server exposing the Ziptastic SDK as
two agent tools — `ziptastic_list` and `ziptastic_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ziptastic-mcp)
make build
...
```jsonc
// ziptastic_list: first page of records
{ "entity": "get_location_by_zipcode" }
{ "entity": "get_location_by_zipcode", "query": { } }

// ziptastic_load: one record by id
{ "entity": "get_location_by_zipcode", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ziptastic-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ziptastic \
     -- "$PWD"/dist/*/ziptastic-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ziptastic_list` and `ziptastic_load` tools now appear
   in new sessions. Ask the agent to *"list get_location_by_zipcode using ziptastic"*
   and it calls `ziptastic_list` with `{"entity":"get_location_by_zipcode"}`.
...
| Tool | Args | Returns |
|------|------|---------|
| `ziptastic_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ziptastic_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
