...
The Lua SDK for the Ziptastic API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:GetLocationByZipcode(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Ziptastic(nil):load(
  { id = "test01" }, nil
)
