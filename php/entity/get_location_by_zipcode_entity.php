...
class GetLocationByZipcodeEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "get_location_by_zipcode";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new GetLocationByZipcodeEntity($this->_client, $opts);
    }

    /**
     * @param GetLocationByZipcode|array $args GetLocationByZipcode data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ZiptasticHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetLocationByZipcode|array The current GetLocationByZipcode data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetLocationByZipcode fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ZiptasticHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetLocationByZipcode fields).
     */
     ...
     *
     * @param GetLocationByZipcodeLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GetLocationByZipcodeLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetLocationByZipcode|array The loaded GetLocationByZipcode as an assoc-array at the
     *   SDK boundary; throws ZiptasticError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ZiptasticHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
