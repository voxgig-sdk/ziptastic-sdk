...
class ZiptasticNetsimFeature extends ZiptasticBaseFeature
{
    ...
    private function _simulate(ZiptasticContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(ZiptasticContext $ctx, array $applied): void
    {
        $client = $this->client;
