...
The Python SDK for the Ziptastic API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.GetLocationByZipcode(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Ziptastic(None).load(
    {"id": "test01"}, None
)
