# Ziptastic SDK test runner

from __future__ import annotations
import os
import json

from ziptastic_sdk.utility.voxgig_struct import voxgig_struct as vs


class ZiptasticTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ZiptasticTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ZiptasticTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ZiptasticTestRunner.getenv("ZIPTASTIC_TEST_LIVE")
        override = ZiptasticTestRunner.getenv("ZIPTASTIC_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ZiptasticTestRunner.getenv(key)
        ...
        explain = ZiptasticTestRunner.getenv("ZIPTASTIC_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ZIPTASTIC_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ZiptasticTestRunner._test_control is not None:
            return ZiptasticTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ZiptasticTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ZiptasticTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ZiptasticTestRunner.load_test_control()
...
def load_env_local():
    ZiptasticTestRunner.load_env_local()


def env_override(m):
    return ZiptasticTestRunner.env_override(m)


def entity_data(v):
    return ZiptasticTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ZiptasticTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ZiptasticTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ZiptasticTestRunner.load_test_control()


def live_client_options():
    return ZiptasticTestRunner.live_client_options()


def live_delay_ms():
    return ZiptasticTestRunner.live_delay_ms()
