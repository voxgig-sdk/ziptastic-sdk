...
class FtCtrl extends ZiptasticControl
{
...
class FtClient
{
    ...
    private ZiptasticUtility $utility;

    public function __construct(ZiptasticUtility $utility, array $options)
    {
    ...
    public function get_utility(): ZiptasticUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ZiptasticContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ZiptasticContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ZiptasticContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ZiptasticConfig::make_feature($name);
    ...
    private static function buildUrl(ZiptasticSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ZiptasticError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ZiptasticResponse($fetched);
        }
    ...
    private function fail(ZiptasticContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ZiptasticContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
