...
module ZiptasticUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = ZiptasticContext.new({}, nil)
    end
    op = ctx.op || ZiptasticOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || ZiptasticResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(ZiptasticError) ? err.msg : err.to_s
    ...
    sdk_err = ZiptasticError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(ZiptasticError)
