# Ziptastic SDK utility: make_request
...
module ZiptasticUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ZiptasticResponse.new({})
    ...
    if fetch_err
      ...
      response = ZiptasticResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ZiptasticResponse.new(fetched)
    else
