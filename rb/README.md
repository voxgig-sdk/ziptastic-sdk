...
The Ruby SDK for the Ziptastic API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.GetLocationByZipcode(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.Ziptastic(nil).load(
  { "id" => "test01" }, nil
)
