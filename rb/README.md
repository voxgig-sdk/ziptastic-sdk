...
The Ruby SDK for the Ziptastic API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.GetLocationByZipcode` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = ZiptasticSDK.test(testopts, sdkopts)
```
...
raise a `ZiptasticError` (a `StandardError` subclass), so wrap
...
```
rb/
├── Ziptastic_sdk.rb       -- Main SDK module
...
The main module (`Ziptastic_sdk`) exports the SDK class
